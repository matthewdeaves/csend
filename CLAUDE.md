...
The project is a thin UI layer on top of the **PeerTalk SDK** (`$PEERTALK_DIR`, default `~/peertalk`), which handles all networking (discovery, connections, messaging) across POSIX, MacTCP, and OpenTransport platforms.
...
6. **Logging**: Uses clog (`$CLOG_DIR`, default `~/clog`) — `CLOG_INFO()`, `CLOG_ERR()`, `CLOG_WARN()`, `CLOG_DEBUG()`
...
- **PeerTalk SDK**: `$PEERTALK_DIR` (default `~/peertalk`) — included via CMake `add_subdirectory()`
- **clog**: `$CLOG_DIR` (default `~/clog`) — minimal C89 logging library
- **Retro68**: `$RETRO68_TOOLCHAIN` (default `~/Retro68-build/toolchain`) — Classic Mac cross-compiler (fork: github.com/matthewdeaves/Retro68)

### Quick Setup

Run `./setup.sh` to bootstrap all dependencies (Retro68, clog, peertalk) from scratch.
