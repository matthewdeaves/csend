...
int start_automated_test(const test_config_t *config, const test_callbacks_t *callbacks)
{
    if (g_test_state.is_running) {
        CLOG_WARN("Test: Cannot start, already in progress");
        return -1;
    }

    memset(&g_test_state, 0, sizeof(g_test_state));
    g_test_state.is_running = 1;
    g_test_state.phase = TEST_PHASE_START_ROUND;
    g_test_state.config = *config;
    g_test_state.callbacks = *callbacks;

    CLOG_INFO("========================================");
    CLOG_INFO("AUTOMATED TEST START");
    CLOG_INFO("Config: rounds=%d, broadcasts=%d, direct_per_peer=%d, delay=%dms",
    ...
    if (g_test_state.peer_count_at_start == 0) {
        CLOG_WARN("Test: No peers available - aborted");
        stop_automated_test();
        return 0;
    }
    CLOG_INFO("Test: Found %d active peer(s)", g_test_state.peer_count_at_start);
...
void stop_automated_test(void)
{
    if (g_test_state.is_running) {
        ...
        duration_ms = ((end_time_ticks - g_test_state.start_time_ticks) * 1000) / 60;
#else
        duration_ms = get_time_ms() - g_test_state.start_time_ms;
#endif
        CLOG_INFO("========================================");
        CLOG_INFO("AUTOMATED TEST END");
        CLOG_INFO("Summary: %d total, %d failed, %d succeeded",
                  g_test_state.total_messages, g_test_state.failed_messages,
                  g_test_state.total_messages - g_test_state.failed_messages);
        CLOG_INFO("Duration: %lu ms", duration_ms);
        CLOG_INFO("========================================");
    }
    memset(&g_test_state, 0, sizeof(g_test_state));
...
void process_automated_test(void)
{
    ...
    if (!g_test_state.is_running) return;

#ifdef __CLASSIC_MAC__
    if (TickCount() < g_test_state.next_step_ticks) return;
#else
    if (get_time_ms() < g_test_state.next_step_ms) return;
#endif
    ...
    case TEST_PHASE_START_ROUND:
        ...
        if (g_test_state.current_round > g_test_state.config.test_rounds) {
            ...
            CLOG_INFO("Test Round %d/%d START", g_test_state.current_round, g_test_state.config.test_rounds);
    ...
    case TEST_PHASE_BROADCASTING:
        ...
        if (g_test_state.current_broadcast_msg > g_test_state.config.broadcast_count) {
            ...
            snprintf(message, sizeof(message), "TEST_R%d_BROADCAST_%d",
                     g_test_state.current_round, g_test_state.current_broadcast_msg);
            CLOG_INFO("Test R%d: Broadcast %d/%d: '%s'",
                      g_test_state.current_round, g_test_state.current_broadcast_msg,
                      g_test_state.config.broadcast_count, message);

            if (g_test_state.callbacks.send_broadcast(message, g_test_state.callbacks.context) != 0) {
                CLOG_WARN("Test R%d: Broadcast %d FAILED", g_test_state.current_round, g_test_state.current_broadcast_msg);
    ...
    case TEST_PHASE_START_DIRECT:
        CLOG_INFO("Test R%d - Direct: %d msg(s) per peer",
    ...
    case TEST_PHASE_DIRECT_MESSAGING:
        ...
        if (g_test_state.current_direct_msg > g_test_state.config.direct_per_peer) {
            ...
            if (g_test_state.callbacks.get_peer_info(
                    g_test_state.current_peer_index,
                    peer_name, sizeof(peer_name),
                    peer_addr, sizeof(peer_addr),
                    g_test_state.callbacks.context) != 0) {
                CLOG_WARN("Test R%d: Failed to get peer %d",
                          g_test_state.current_round, g_test_state.current_peer_index);
                g_test_state.current_peer_index++;
                g_test_state.current_direct_msg = 0;
                schedule_next_step(0);
                break;
            ...
            snprintf(message, sizeof(message), "TEST_R%d_DIRECT_%d_TO_%s_MSG_%d",
                     g_test_state.current_round, g_test_state.current_peer_index + 1,
                     peer_name, g_test_state.current_direct_msg);

            CLOG_INFO("Test R%d: Direct %d/%d to %s: '%s'",
                      g_test_state.current_round, g_test_state.current_direct_msg,
                      g_test_state.config.direct_per_peer, peer_name, message);

            if (g_test_state.callbacks.send_direct(
                    g_test_state.current_peer_index, message,
                    g_test_state.callbacks.context) != 0) {
                CLOG_WARN("Test R%d: Direct to %s FAILED", g_test_state.current_round, peer_name);
                g_test_state.failed_messages++;
            }
            g_test_state.total_messages++;
            schedule_next_step(0);
    ...
    case TEST_PHASE_END_ROUND:
        CLOG_INFO("Test Round %d/%d COMPLETE", g_test_state.current_round, g_test_state.config.test_rounds);
        g_test_state.phase = TEST_PHASE_START_ROUND;
    ...
    default:
        CLOG_WARN("Test: Unknown phase %d", (int)g_test_state.phase);
        stop_automated_test();
        break;
    }
}
...
#else
    g_test_state.next_step_ms = get_time_ms() + (unsigned long)delay_ms;
#endif
}
