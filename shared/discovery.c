                                    ...
                                    void *platform_context)
{
    ...
    if (strcmp(msg_type, MSG_DISCOVERY) == 0) {
        ...
        if (add_result > 0) {
            log_info_cat(LOG_CAT_DISCOVERY, "New peer added via DISCOVERY: %s@%s", sender_username, sender_ip_str);
            callbacks->notify_peer_list_updated_callback(platform_context);
        } else if (add_result == 0) {
            log_debug_cat(LOG_CAT_DISCOVERY, "Existing peer updated via DISCOVERY: %s@%s", sender_username, sender_ip_str);
        } else {
            log_warning_cat(LOG_CAT_DISCOVERY, "Peer list full, could not add %s@%s from DISCOVERY", sender_username, sender_ip_str);
        }
    } else if (strcmp(msg_type, MSG_DISCOVERY_RESPONSE) == 0) {
        log_debug_cat(LOG_CAT_DISCOVERY, "Received DISCOVERY_RESPONSE from %s@%s", sender_username, sender_ip_str);
        add_result = callbacks->add_or_update_peer_callback(sender_ip_str, sender_username, platform_context);
        if (add_result > 0) {
            log_info_cat(LOG_CAT_DISCOVERY, "New peer added via RESPONSE: %s@%s", sender_username, sender_ip_str);
            callbacks->notify_peer_list_updated_callback(platform_context);
        } else if (add_result == 0) {
            log_debug_cat(LOG_CAT_DISCOVERY, "Existing peer updated via RESPONSE: %s@%s", sender_username, sender_ip_str);
        } else {
